CREATE OR REPLACE VIEW neo_job.vw_job_list (
    id,
    job_title,
    job_code,
    job_description,
...
GROUP BY candidates_jobs.job_id
    ...
    j.no_of_position,
    j.customer_id,
    COALESCE(cust.customer_name, j.customer_name) AS customer_name,
    j.customer_manager,
    ...
    FROM neo_job.jobs j
     LEFT JOIN neo_customer.customers cust ON cust.id = j.customer_id
