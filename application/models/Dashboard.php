...
class Dashboard extends MY_Model
{
  ...
  public function assignedJobs() {
    ...
    return array_column($job_ids, 'job_id');
  }
 ...
 /******* SUMIT END *************/
  ...
  public function getOfferedCandidatesCount() {
    ...
    public function getDraftedJobsCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('job_status_id', [1])->from('neo_job.jobs')->count_all_results());
    }

    public function getOpenJobsCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('job_status_id', [2])->from('neo_job.jobs')->count_all_results());
    }

    public function getClosedJobsCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('job_status_id', [3])->from('neo_job.jobs')->count_all_results());
    }

    public function getOnHoldJobsCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('job_status_id', [4])->from('neo_job.jobs')->count_all_results());
    }
    ...
    public function getLeadIdentifiedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [1])->from('neo_customer.customers')->count_all_results());
    }

    public function getInitialMeetingScheduleCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [2])->from('neo_customer.customers')->count_all_results());
    }

    public function getInitialMeetingCompletedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [3])->from('neo_customer.customers')->count_all_results());
    }

    public function getOpLostAtEntryLevelCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [4])->from('neo_customer.customers')->count_all_results());
    }

    public function getFollowUpMeetingScheduleCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [5])->from('neo_customer.customers')->count_all_results());
    }

    public function getFollowUpMeetingCompletedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [6])->from('neo_customer.customers')->count_all_results());
    }

    public function getOpLostAtFollowUpLevelCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [7])->from('neo_customer.customers')->count_all_results());
    }

    public function getProposalSharedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [8])->from('neo_customer.customers')->count_all_results());
    }

    public function getProposalUnderReviewCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [9])->from('neo_customer.customers')->count_all_results());
    }

    public function getProposalUnderRFECount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [10])->from('neo_customer.customers')->count_all_results());
    }

    public function getNegotiationCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [11])->from('neo_customer.customers')->count_all_results());
    }

    public function getProposalAcceptedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [12])->from('neo_customer.customers')->count_all_results());
    }

    public function getOpLostAtProposalLevelCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [13])->from('neo_customer.customers')->count_all_results());
    }

    public function getContractSignedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [14])->from('neo_customer.customers')->count_all_results());
    }

    public function getContractNotSignedCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [15])->from('neo_customer.customers')->count_all_results());
    }

    public function getLeadConvertToClientCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [16])->from('neo_customer.customers')->count_all_results());
    }

    public function getOnHoldCount() {
      return $this->IND_money_format($this->db->where_in('created_by', $this->hierarchy())->where_in('lead_status_id', [17])->from('neo_customer.customers')->count_all_results());
    }
