...
*/
    ...
    public function get_candidates_by_qp($qp_id = 0, $job_status_id = 0)   //changed to view
    {
        ...
        $qp_rec = $this->db->query("SELECT name AS qualification_pack_name FROM neo_master.qualification_packs WHERE id=$qp_id");
        ...
        $candidate_det_rec = $this->db->query("SELECT 		CJ.id AS job_applied_id,
                                                            ...
                                                            COALESCE(NULLIF(J.job_location,''),'-NA') as location_name,
   ...
   function get_jobs_list_new($requestData = array(), $user_id = 0)
    {
        ...
        $columns = array(
            ...
            2 => 'VW.job_title',
            3 => 'VW.customer_name',
            4=> 'VW.job_status_name',
            5 => 'VW.created_at',
            6 => 'VW.no_of_position',
            7 => 'VW.qualification_pack_name',
            8 => 'VW.business_vertical_name',
            9 => 'VW.functional_area_name',
            10 => 'VW.industry_name',
            11 => 'VW.job_location',
            12 => 'VW.education_name',
            13 => 'VW.job_open_type_name',
            14 => 'VW.job_priority_level_name',
            15=>  'VW.job_expiry_date'
        );

        $column_search = array('VW.job_title', 'VW.customer_name', 'VW.created_at', 'VW.no_of_position', 'VW.qualification_pack_name', 'VW.business_vertical_name', 'VW.functional_area_name', 'VW.industry_name', 'VW.job_location', 'VW.education_name', 'VW.job_open_type_name', 'VW.job_priority_level_name', 'VW.job_status_name', 'VW.job_expiry_date' );
        ...
        {
           $totalFiltered = $this->db->query("SELECT Count(*) AS total_filtered FROM neo_job.vw_job_list AS VW  WHERE TRUE $sWhere $HierarchyCondition")->row()->total_filtered;
...
//               }
   ...
   function get_assigned_jobs_list_new($requestData=array(),$user_id = 0)
   {
    ...
    function get_address_book_contact_list($PageRequestData = array(),$user_id=0)
    {
        ...
        //Searching columns
        $arrColumnsToBeSearched = array("customer_name", "spoc_name" ,"spoc_phone","spoc_email" );
    ...
    function get_batches_list($PageRequestData = array())
    {
        ...
        //Searching columns
        $arrColumnsToBeSearched = array("batch_code","batch_size","project_name", "center_name" ,"program_name", "client_name","sbu","trainer_email_id","bc_start_date","bc_end_date" );
        ...
        if (!$intTotalRecordCount) {
            ...
            foreach ($QueryData->result() as $QueryRow) {
                ...
                $ResponseRow[] = $QueryRow->project_name ?? 'N/A';
                $ResponseRow[] = $QueryRow->center_name ?? 'N/A';
                $ResponseRow[] = $QueryRow->program_name ?? 'N/A';
                $ResponseRow[] = $QueryRow->client_name ?? 'N/A';
                $ResponseRow[] = $QueryRow->sbu ?? 'N/A';
                $ResponseRow[] = $QueryRow->trainer_email_id ?? 'N/A';
                $ResponseRow[] = $QueryRow->bc_start_date ?? 'N/A';
                $ResponseRow[] = $QueryRow->bc_end_date ?? 'N/A';
                $Data[] = $ResponseRow;
            }
 ...
 public function get_candidates_by_batchwise($id = 0)   //changed to view
 {
        ...
        $batch_rec = $this->db->where("id",$id)->get("neo.batches")->row();
        if (!empty($batch_rec))
        {
            $BatchCode = $batch_rec->batch_code;
            $ProjectName = $batch_rec->project_name;
            $ProgramName = $batch_rec->program_name;
            $ClientName = $batch_rec->client_name;
            $CenterName = $batch_rec->center_name;
            $BatchStartDate = $batch_rec->bc_start_date;
            $BatchEndDate = $batch_rec->bc_end_date;
            $BusinessUnit = $batch_rec->sbu;
            $TrainerEmail = $batch_rec->trainer_email_id;

            $output['status'] = true;
            $output['batch_code'] = $BatchCode;
            $output['project_name'] = $ProjectName;
            $output['program_name'] = $ProgramName;
            $output['client_name'] = $ClientName;
            $output['center_name'] = $CenterName;
            $output['bc_start_date'] = $BatchStartDate;
            $output['bc_end_date'] = $BatchEndDate;
            $output['sbu'] = $BusinessUnit;
            $output['trainer_email_id'] = $TrainerEmail;
        }
        ...
        if ($BatchCode != '')
        {
            $candidate_det_rec = $this->db->query("SELECT   	DISTINCT
                                                ...
                                                LEFT JOIN 	neo.batches AS B ON UPPER(TRIM(B.batch_code))=UPPER(TRIM(CB.batch_code))
                                                WHERE   	B.id=$id
                                                ORDER BY	2");
            if ($candidate_det_rec->num_rows())
            {
