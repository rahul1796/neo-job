...
#screening_job_content_block div.row
{
    margin-bottom: 5px;
    padding: 5px;
    border-radius: 10px;
    ...
    <section id="description" class="card" style="border: none!important;">
        <div class="col-md-12">
            <div class="card">
                ...
                <a href="<?php echo base_url("partner/scheduled_jobs/".$candidate_detail['candidate_id']);?>" style="float: right; margin-top: -43px;"><button type="button" class="btn btn-success btn-min-width mr-1 mb-1"><i class="icon-android-calendar"></i>Jobs Screened For</button></a>
                <div class="card-body">
                    <table class="table table-bordered">
                        <form name="form_screening_jobs" method="post" action="<?php echo base_url('partner/update_screened_job_status')?>">
                            ...
                            <div id="screening_job_list_block" class="page_content table-responsive" style="overflow-x: hidden;">
                                <div id="screening_job_content_block">
...
/*  $(".date_from").datepicker({'changeYear':true,'changeMonth':true});
  ...
  */
  load_screening_job_list_content('');
  //check box toggling
...
$('#screening_job_list_block .pagination').on('click','a',function(e)
{
  e.preventDefault();
  load_screening_job_list_content($(this).attr('href'));
});
...
function reload_job_list()
{
  ...
  if(candidate_id=='')
      candidate_id=0;
  pgurl=site_url+'partner/screening_jobs_list/'+candidate_id+'/'+sel_page;
  if(pgurl != '')
  {
    load_screening_job_list_content(pgurl);
  }
return false;
}
 ...
 */
function load_screening_job_list_content(pagi_url)
{
  ...
  if(pagi_url == '')
  {
    url = site_url+'partner/screening_jobs_list/'+candidate_id+'/'+sel_page;
  }
  else
    url = pagi_url; 

  $('#screening_job_list_block #screening_job_content_block').html('<div class="row"><div class="col-sm-12 col-md-12 col-lg-12" align="center" style="margin:5px;padding:5px;"><img src="'+base_url+'assets/images/loading_bar.gif'+'"></div></div>');
  $('#screening_job_list_block .pagination').html('');
  $.getJSON(url,'',function(resp)
  {
    var screening_job_list_html = '';
    var page_display_log='';
    if(resp==null) 
    {
      screening_job_list_html='<div class="row"><div class="col-sm-12 col-md-12 col-lg-12"  align="center">Unable to load the content, Please <a href="javascript:void(0)" onclick="return load_screening_job_list_content(\'\');">Click here to Reload</a>.</div></div>';
    }
    else if(resp.status == 'success')
    {

      screening_job_list_html = screening_job_list_inner_content(resp,colcount);
    ...
    {
      screening_job_list_html += '<div class="row"><div class="col-sm-12 col-md-12 col-lg-12"  align="center">'+resp.message+'</div></div>';
    }
   
    $('#screening_job_list_block #screening_job_content_block').html(screening_job_list_html);
    $('.page_display_log').html(page_display_log);

  });
}

function screening_job_list_inner_content(resp,colcount)
{
  var  job_list_html='';
  var page_no=resp.pg;
      ...
      job_list_html += '  <div class="row">';
          ...
          job_list_html += '  <div class="col-sm-7 col-md-7">';
              job_list_html += '<p><b>Employer: '+b.employer_name+'</b></p>'+
                        ...
                        '<li><b>Question 1 :</b> '+b.question_one+
                            '<br>'+
                            '<span><input type="radio" name="radio_q1_'+b.id+'" '+status_yes[b.q1_response]+' value="1" /><b>Yes :</b> <input type="radio" name="radio_q1_'+b.id+'" '+status_no[b.q1_response]+' value="0" /> <b> No :</b> </span>'+
                        '</li>'+
                        '<li><b>Question 2 :</b> '+b.question_two+
                            '<br>'+
                            '<span> <input type="radio" name="radio_q2_'+b.id+'" '+status_yes[b.q2_response]+' value="1" /> <b>Yes :</b> <input type="radio" name="radio_q2_'+b.id+'" '+status_no[b.q2_response]+' value="0" /> <b> No :</b> </span>'+
                        '</li>'+
                        '</ul>';
          job_list_html += '</div> ';
          ...
          if (Math.abs(b.status_id)<3)
          {
            ...
            job_list_html += '<br><br><a class="btn btn-block btn-primary '+submit_btn_status+'" href="javascript:void(0)" title="Submit" onclick="screened_job_status('+b.id+')" >Submit</a>';
            job_list_html += '<p class="postedon small">Posted on:'+b.created_on+'</p>';
            job_list_html += '  </div> ';
          }
          else
          {
             job_list_html += b.job_status;
          }
      job_list_html += '  </div> ';   
...
function screened_job_status(screened_job_id)
{
