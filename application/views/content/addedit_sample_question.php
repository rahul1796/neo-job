...
<div class="content-body" style="overflow-x: hidden;"><!-- Basic form layout section start -->
	<?php
	$QuestionImagePath = base_url(SAMPLE_QUESTION_IMAGES);
	...
	echo '<a class="btn btn-success btn-min-width mr-1 mb-1" href="' . base_url('content/sample_questions') . '" style="color: white; float:right;"><i class="icon-eye"></i> Preview Sample Question Paper</a>';
	?>
							...
							<form class="form form-horizontal form-bordered" id="frmEntry" method="post" enctype="multipart/form-data">
								...
								<div class="form-body" style="margin-top: -25px;">
									<div id="divQuestionPaperTitle"  class="form-group row">
										...
										<div class="col-sm-9">
											<input type="text" class="form-control" id="txtQuestionPaperTitle" name="txtQuestionPaperTitle" value="Sample Question Paper" onkeydown="return false;"/>
										</div>
									</div>
									...
									<div id="divLanguage"  class="form-group row" style="display:block;">
										...
										<div class="col-sm-9">
											<input type="text" class="form-control" id="txtLanguage" name="txtLanguage" value="<?= $LanguageName ?>" onkeydown="return false;"/>
										</div>
									</div>
...
$("#frmEntry").on('submit',(function(e)
{
	...
	$.ajax({
		url: base_url + "content/SaveSampleQuestionData",
		type: "POST",
		...
		success: function (result)
		{
			...
			function (confirmed)
			{
				if (parseInt(result[0].o_question_id) > 0)
					window.location.href = base_url + "content/sample_questions";
			});
		},
		error: function ()
		{
