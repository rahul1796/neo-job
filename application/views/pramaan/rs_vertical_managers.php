 ...
 * @desc  recruitment support - Vertical Manager List
 * @date  Nov_2016
*/
select.input-sm 
{
...
<div class="content-body" style="overflow-x: hidden !important;">
    <?php
    if (intval($user_group_id) == 14)
        echo '<a class="btn btn-success btn-min-width mr-1 mb-1" href="' . base_url("pramaan/addedit_rs_vertical_manager") . '"><i class="icon-android-add"></i> Add RS Vertical Manager</a>';
    ?>
    ...
    <div class=" breadcrumbs-top col-md-9 col-xs-12">
        <div class="breadcrumb-wrapper col-xs-12">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Placement Support Vertical Managers
                </li>
            </ol>
        </div>
    </div>
    <div id="divSectorManagers" class="modal fade bs-example-modal-lg" role="dialog">
        <div class="modal-dialog modal-lg" role="document" style="width:75%;" >
            <div class="modal-content">
                <div class="modal-header" style="border-bottom:hidden;">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h3 class="modal-title">Placement Sector Manager List</h3>
                </div>
                <div class="modal-body">
                    <table id="tblSectorManager" class="table table-striped" cellspacing="0" style="width:100%; !important;">
                        <thead>
                        <tr>
                            <th>SNo.</th>
            ...
            <div class="col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Available Placement Support Vertical Managers </h4>
                    ...
                    <div class="card-body collapse in">
                        <div class="card-block card-dashboard">
                            ...
                            <table id="tblVerticalManager" class="table table-striped table-bordered display responsive nowrap" style="width:100% !important;">
                                <thead>
                                <tr>
                                    <th>SNo.</th>
                                    ...
                                    <th>Email</th>
                                    <th>Verticals</th>
                                    <th>Placement Head</th>
                                    <th>Sector Managers</th>
                                    <th>Status </th>
                                    <th>Actions </th>
                                </tr>
                                </thead>
...
$(document).ready(function() {
    table = $('#tblVerticalManager').DataTable({
        "serverSide": true,
        "paging": true,
        "scrollX": true,
        ...
        "ajax": {
            "url": base_url + "pramaan/rs_vertical_manager_list/",
            "type": "POST",
            error: function()
            {
                $("#tblVerticalManager tbody").empty().append('<tr><td style="text-align: center;" colspan="9">No data found</td></tr>');
            }
        },
        "columnDefs":
            [
...
function ViewSectorManagerList(RsVerticalManagerId) {
    if (RsVerticalManagerId) {
        $.ajax({
            type: "POST",
            url: base_url + "pramaan/get_sector_managers_for_rs_vertical_manager",
            data: {
                'id': RsVerticalManagerId
            },
            dataType: 'json',
            success: function (data) {
                ...
                for(var i = 0; i < data.length; i++)
                {
                    ...
                    varTableBodyHtml += "<td>" + data[i]['sector_manager_name'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['sector_manager_phone'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['sector_manager_email'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['sector_name_list'] + "</td>";
                    varTableBodyHtml += "</tr>";
                }

                $("#tblSectorManagerBody").empty().append(varTableBodyHtml);
                $("#divSectorManagers").modal({ show: true });
            },
            error: function () {
...
function EditRSVerticalManager(RsVerticalManagerId)
{
    if (RsVerticalManagerId) {
        document.location.href = base_url + 'pramaan/addedit_rs_vertical_manager/' + RsVerticalManagerId;
    }
}
    ...
    swal(
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: base_url + "pramaan/change_rs_vertical_manager_active_status",
                    data: {
                        'id': id,
