...
<div class="content-body" style="overflow-x: hidden !important;">
    <a href="<?php echo base_url("pramaan/add_bd_admin")?>"><button type="button" class="btn btn-success btn-min-width mr-1 mb-1"><i class="icon-android-add"></i>Add BD Admin</button></a>
    <!-- File export table -->
    <div class=" breadcrumbs-top col-md-10 col-xs-12">
        <div class="breadcrumb-wrapper col-xs-12">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Recruitment Admins
                </li>
            </ol>
        </div>
    </div>
            ...
            <div class="col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Available Recruitment Admin</h4>
                    ...
                    <div class="card-body collapse in">
                        <div class="card-block card-dashboard">
                            ...
                            <table id="tblBdAdmin" class="table table-striped table-bordered" style="width:100% !important;">
                                <thead>
                                <tr>
                                    <th>SNo</th>
                                    <th>Recruitment Admin</th>
                                    <th>Email</th>
                                    <th>Phone</th>
                                    <th>BD Head</th>
                                    <th>Status</th>
...
$(document).ready(function() {
    table = $('#tblBdAdmin').DataTable({
       // "stateSave": true,
        ...
        "ajax": {
            "url": base_url + "pramaan/bd_admins_list/"+"<?php echo $user_id;?>"+"/"+"<?php echo $user_group_id;?>",
            "type": "POST",
            error: function()
            {
                $("#tblBdAdmin tbody").empty().append('<tr><td style="text-align: center;" colspan="9">No data found</td></tr>');
            }
        },
        "columnDefs":
            [
   ...
   <?php
    if (intval($user_group_id) == 10) {
        echo 'table.columns([4,5,6]).visible(false);';
    }
...
function EditBdAdmin(Id)
{
    if (Id) {
        document.location.href = base_url + 'pramaan/edit_bd_admin/'+'<?php echo $user_id;?>'+'/'+Id;
    }
}
    ...
    swal(
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: base_url + "pramaan/change_bd_admin_active_status",
                    data: {
                        'id': id,
