...
<div class="content-body" style="overflow-x: hidden !important;">
    <a href="<?php echo base_url("pramaan/add_bd_executive/$parent_id")?>"><button type="button" class="btn btn-success btn-min-width mr-1 mb-1"><i class="icon-android-add"></i>Add BD Executive</button></a>
      <!-- File export table -->
    <div class=" breadcrumbs-top col-md-9 col-xs-12">
        <div class="breadcrumb-wrapper col-xs-12">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Recruitment Executive
                </li>
            </ol>
        </div>
    </div>
    <div id="divBdEmp" class="modal fade bs-example-modal-lg" role="dialog">
         <div class="modal-dialog modal-lg" role="document" style="width:75%;" >
             <div class="modal-content">
                 <div class="modal-header" style="border-bottom:hidden;">
                     <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                     <h3 class="modal-title">Employers List</h3>
                 </div>
                 <div class="modal-body">
                     <table id="tblBdEmp" class="table table-striped" cellspacing="0" style="width:100%; !important;">
                         <thead>
                         <tr>
                             <th>SNo.</th>
                             <th>Name</th>
            ...
            <div class="col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Available Recruitment Executive</h4>
                    ...
                    <div class="card-body collapse in">
                        <div class="card-block card-dashboard">
                            ...
                            <table id="tblBdExecutive" class="table table-striped table-bordered" style="width:100% !important;">
                                <thead>
                                <tr>
                                    <th>SNo</th>
                                    <th>Name</th>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <center><h4 class="modal-title"><b>Employers</b></h4></center>
        </div>
...
$(document).ready(function() {
    table = $('#tblBdExecutive').DataTable({
        "stateSave": true,
        "serverSide": true,
        "paging": true,
        "scrollX": true,
        ...
        "ajax": {
            "url": base_url + "pramaan/get_bd_executive_list/"+"<?php echo $parent_id;?>",
            "type": "POST",
            error: function()
            {
                $("#tblBdExecutive tbody").empty().append('<tr><td style="text-align: center;" colspan="9">No data found</td></tr>');
            }
        },
        "columnDefs":
            [
...
function ViewEmployerList(Id) {
    $("#divBdEmp").modal({ show: true });
    if (Id) {
        $.ajax({
            type: "POST",
            url: base_url + "pramaan/get_bd_emp_for_bd_executive",
            data: {
                'id': Id
            },
            dataType: 'json',
            success: function (data) {
                ...
                for(var i = 0; i < data.length; i++)
                {
                    ...
                    varTableBodyHtml += "<td>" + data[i]['email'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['phone'] + "</td>";
                    varTableBodyHtml += "</tr>";
                }

                $("#tblBdEmpBody").empty().append(varTableBodyHtml);
                $("#divBdEmp").modal({ show: true });
            },
            error: function () {
...
function EditBdExecutive(Id)
{
    if (Id) {
        document.location.href = base_url + 'pramaan/edit_bd_executive/' +"<?php echo $parent_id;?>"+"/"+ Id;
    }
}
    ...
    swal(
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: base_url + "pramaan/change_bd_executive_active_status",
                    data: {
                        'id': id,
...
function get_emp_list(user_id)
{
   /* alert(user_id);*/
   $.ajax({
        url : base_url+"pramaan/get_employers_list/"+user_id,
        type: "POST",
        dataType: "JSON",
            ...
            if (data.status == true) 
              {
               ...
               window.location.href = base_url+'pramaan/bd_executives';
