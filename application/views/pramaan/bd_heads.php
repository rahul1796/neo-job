...
<div class="content-body" style="overflow-x: hidden !important;">
    <a href="<?php echo base_url("pramaan/add_bd_head/$parent_id")?>"><button type="button" class="btn btn-success btn-min-width mr-1 mb-1"><i class="icon-android-add"></i> Add BD Head</button></a>
    <!-- File export table -->
    <div class=" breadcrumbs-top col-md-10 col-xs-12">
        <div class="breadcrumb-wrapper col-xs-12">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Recruitment heads
                </li>
            </ol>
        </div>
    </div>
    <div id="divBdRegionalManagers" class="modal fade bs-example-modal-lg" id="modal_form" role="dialog">
        <div class="modal-dialog modal-lg" role="document" style="width:75%;" >
            <div class="modal-content">
                <div class="modal-header" style="border-bottom:hidden;">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h3 class="modal-title">Recruitment Regional Manager List</h3>
                </div>
                <div class="modal-body">
                    <table id="tblRegionalManager" class="table table-striped" cellspacing="0" style="width:100%; !important;">
                        ...
                        <tbody id="tblBdRegionalManagerBody">
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Available Recruitment Heads</h4>
                    ...
                    <div class="card-body collapse in">
                        <div class="card-block card-dashboard">
                            ...
                            <table id="tblBdHead" class="table table-striped table-bordered" style="width:100% !important;">
                                <thead>
                                <tr>
                                    <th>SNo</th>
                                    <th style="text-align: center;">Recruitment head name</th>
                                    <th style="text-align: center;">Phone</th>
                                    <th style="text-align: center;">Email</th>
                                    <th style="text-align: center;">Regional Managers</th>
...
$(document).ready(function() {
    table = $('#tblBdHead').DataTable({
        ...
        "ajax": {
            "url": base_url + "pramaan/bd_heads_list/",
            "type": "POST",
            error: function()
            {
                $("#tblBdHead tbody").empty().append('<tr><td style="text-align: center;" colspan="9">No data found</td></tr>');
            }
        },
        "columnDefs":
            [
        ...
        buttons:
            [
                /*{
                    ...
                    pageSize: 'A4',
                    title: 'Business Development Heads',
...
function ViewBdRegionalManagerList(BdHeadId) {
    $("#divBdRegionalManagers").modal({ show: true });
    if (BdHeadId) {
        $.ajax({
            type: "POST",
            url: base_url + "pramaan/get_bd_regional_managers_for_bd_head",
            data: {
                'id': BdHeadId
            },
            dataType: 'json',
            success: function (data) {
                ...
                for(var i = 0; i < data.length; i++)
                {
                    ...
                    varTableBodyHtml += "<td>" + data[i]['bd_regional_manager_name'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['bd_regional_manager_phone'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['bd_regional_manager_email'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['region_name_list'] + "</td>";
                    varTableBodyHtml += "</tr>";
                }

                $("#tblBdRegionalManagerBody").empty().append(varTableBodyHtml);
                $("#divBdRegionalManagers").modal({ show: true });
            },
            error: function () {
...
function EditBdHead(Id)
{
    if (Id) {
        document.location.href = base_url + 'pramaan/edit_bd_head/' +"<?php echo $parent_id;?>"+"/"+ Id;
    }
}
    ...
    swal(
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: base_url + "pramaan/change_bd_head_active_status",
                    data: {
                        'id': id,
