    ...
    <div class=" breadcrumbs-top col-md-9 col-xs-12">
        <div class="breadcrumb-wrapper col-xs-12">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Sourcing Partners
                </li>
            </ol>
        </div>
    </div>
    <div id="divCenter" class="modal fade bs-example-modal-lg" role="dialog">
        <div class="modal-dialog modal-lg" role="document" style="width:75%;" >
            <div class="modal-content">
                <div class="modal-header" style="border-bottom:hidden;">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h3 class="modal-title">Center List</h3>
                </div>
                <div class="modal-body">
                    <table id="tblCenter" class="table table-striped" cellspacing="0" style="width:100%; !important;">
                        <thead>
                        <tr>
                            <th>SNo.</th>
                            <th>Name</th>
                ...
                <div class="card">
                    ...
                    <div class="card-body collapse in">
                        <div class="card-block card-dashboard">
                            ...
                            <table id="tblSP" class="table table-striped table-bordered" style="width:100% !important;">
                                <thead>
                                <tr>
                                    <th>SNo</th>
                                    <th>Name</th>
...
$(document).ready(function() {
    table = $('#tblSP').DataTable({
        ...
        "ajax": {
            "url": base_url + "pramaan/sourcing_partner_list/"+"<?php echo $district_coordinator_id;?>",
            "type": "POST",
            error: function()
            {
                $("#tblSP tbody").empty().append('<tr><td style="text-align: center;" colspan="9">No data found</td></tr>');
            }
        },
        "columnDefs":
            [
...
function ViewCenterList(Id) {
    $("#divRegionalManagers").modal({ show: true });
    if (Id) {
        $.ajax({
            type: "POST",
            url: base_url + "pramaan/centers_list_by_sourcing_partner",
            data: {
                'id': Id
            },
            dataType: 'json',
            success: function (data) {
                    ...
                    for(var i = 0; i < data.length; i++)
                        {
                            ...
                            varTableBodyHtml += "<td>" + data[i]['address'] + "</td>";
                            if(data[i]['associate_name_list'])
                              varTableBodyHtml += "<td>" + data[i]['associate_name_list'] + "</td>";
                            else
                              varTableBodyHtml += "<td>Not Assigned</td>";
                ...
                {
                             varTableBodyHtml += "<center><h4>No Data</h4></center>";
                }
              
                $("#tblCenterBody").empty().append(varTableBodyHtml);
                $("#divCenter").modal({ show: true });
            },
            error: function () {
...
function EditSourcingPartner(Id)
{
    if (Id) {
        document.location.href = base_url + 'pramaan/add_sourcing_partner/' +"<?php echo $district_coordinator_id;?>"+"/"+ Id;
    }
}
    ...
    swal(
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: base_url + "pramaan/change_sr_partner_active_status",
                    data: {
                        'id': id,
