			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<h4 class="card-title" id="bordered-layout-basic-form"><?= (($id > 0) ? "Edit " : "Add ") . $title . ((($id > 0) ? (isset($ResponseData[0]['rs_head_name']) ? (" - " . $ResponseData[0]['rs_head_name']) : "")  : "")) ?></h4>
							...
							<form class="form form-horizontal form-bordered" id="frmEntry" method="post">
								<div class="form-body">
									<div class="form-group row">
										<input type="hidden" id="hidRsHeadId" name="hidRsHeadId" value="<?= isset($ResponseData[0]['rs_head_id']) ? $ResponseData[0]['rs_head_id'] : "0" ?>"/>
										<label class="col-md-3 label-control" for="pname">Name<span class='validmark'>*</span></label>
										<div class="col-md-9">
											<input type="text" class="form-control" id="txtRsHeadName" name="txtRsHeadName" value="<?= isset($ResponseData[0]['rs_head_name']) ? $ResponseData[0]['rs_head_name'] : "" ?>" placeholder="Enter RS Head Name"/>
									...
									<div class="form-group row">
										...
										<div class="col-md-9">
											<input type="text" class="form-control" id="txtRsHeadEmail" name="txtRsHeadEmail" maxlength="<?= EMAIL_MAX?>" value="<?= isset($ResponseData[0]['rs_head_email']) ? $ResponseData[0]['rs_head_email'] : "" ?>" placeholder="Enter RS Head Email"/>
											<span class="error_label"></span>
										</div>
									</div>
									...
									<div class="form-group row">
										...
										<div class="col-md-9">
											<input type="text" class="form-control" id="txtRsHeadPhone" name="txtRsHeadPhone" maxlength="<?= PHONE_MAX?>" value="<?= isset($ResponseData[0]['rs_head_phone']) ? $ResponseData[0]['rs_head_phone'] : "" ?>" placeholder="Enter RS Head Phone"/>
											<span class="error_label"></span>
										</div>
									</div>
...
$(document).ready(function()
{
	$("#frmEntry").validate({
		...
		submitHandler: function (form) 
	 	{
	 		var id = $("#hidRsHeadId").val();
			var name = $("#txtRsHeadName").val();
			var email = $("#txtRsHeadEmail").val();
			var phone = $("#txtRsHeadPhone").val();
	    	...
	    	$.ajax({
				type: "POST",
				url: base_url + "pramaan/save_rs_head_detail",
				data:
				{
					'id' : id,
					'name' : name,
				...
				success: function (data)
				{
					if (data.status)
					{
						...
						function (confirmed) {
							window.location.href = base_url + "pramaan/rs_heads/";
						});
					}
					else
					{
