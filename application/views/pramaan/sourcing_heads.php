...
.searchprint .btn-group
{
  padding-bottom: 5px;
    ...
    <div class="content-body" style="overflow-x: hidden !important;">
        <a href="<?php echo base_url("pramaan/add_sourcing_head/$parent_id")?>"><button type="button" class="btn btn-success btn-min-width mr-1 mb-1" style="margin-top: 10px;"><i class="icon-android-add"></i> Add Sourcing Head</button></a>
        <!-- File export table -->
        <div class=" breadcrumbs-top col-md-9 col-xs-12" style="top: 10px;">
        ...
        <div id="divRegionalManagers" class="modal fade bs-example-modal-lg" role="dialog">
                ...
                <div class="col-xs-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Available Sourcing Heads</h4>
                        ...
                        <div class="card-body collapse in">
                            <div class="card-block card-dashboard">
                                ...
                                <table id="tblSrHead" class="table table-striped table-bordered" style="width:100% !important;">
                                    <thead>
                                    <tr>
                                        <th>Sl.No.</th>
                                        <th>Name</th>
...
$(document).ready(function() {
    table = $('#tblSrHead').DataTable({
        ...
        "ajax": {
            "url": base_url + "pramaan/sourcing_heads_list/",
            "type": "POST",
            error: function()
            {
                $("#tblSrHead tbody").empty().append('<tr><td style="text-align: center;" colspan="9">No data found</td></tr>');
            }
        },
        "columnDefs":
            [
...
function ViewRegionalManagerList(SrHeadId) {
    $("#divRegionalManagers").modal({ show: true });
    if (SrHeadId) {
        $.ajax({
            type: "POST",
            url: base_url + "pramaan/get_regional_managers_for_sr_head",
            data: {
                'id': SrHeadId
            },
            dataType: 'json',
            success: function (data) {
                ...
                for(var i = 0; i < data.length; i++)
                {
                    ...
                    varTableBodyHtml += "<td>" + data[i]['regional_manager_name'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['regional_manager_phone'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['regional_manager_email'] + "</td>";
                    varTableBodyHtml += "<td>" + data[i]['region_name_list'] + "</td>";
                    varTableBodyHtml += "</tr>";
                }

                $("#tblRegionalManagerBody").empty().append(varTableBodyHtml);
                $("#divRegionalManagers").modal({ show: true });
            },
            error: function () {
...
function EditSourcingHead(SrHeadId)
{
    if (SrHeadId) {
        document.location.href = base_url + 'pramaan/edit_sourcing_head/' +"<?php echo $parent_id;?>"+"/"+ SrHeadId;
    }
}
    ...
    swal(
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: base_url + "pramaan/change_sr_head_active_status",
                    data: {
                        'id': id,
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Sourcning Manager Form</h3>
            </div>
                ...
                <form action="#" id="form_sourcing_admin" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            <label class="control-label col-md-3">Sourcing Manager Name<span class='validmark'>*</span></label>
                            <div class="col-md-9">
                                <input name="pname" placeholder="Sourcing Admin Name" class="form-control" type="text">
